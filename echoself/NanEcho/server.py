...
A FastAPI server that provides a chat interface to a Deep Tree Echo-trained nanoGPT model.
Features include:
- Standard chat completion endpoints
- Streaming response support
- Introspective diagnostics for Deep Tree Echo systems
...
app = FastAPI(
    title="NanEcho API",
    description="API for interacting with a Deep Tree Echo-trained nanoGPT model",
    version="0.1.0",
    lifespan=lifespan
)
...
async def root():
    """
    Returns basic information about the NanEcho API, including its name, description, status, and version.
    """
    return {
        "name": "NanEcho API",
        "description": "API for interacting with a Deep Tree Echo-trained nanoGPT model",
...
async def run_diagnostics(
    ...
    """
    Performs introspective diagnostics on a Deep Tree Echo agent using AtomSpace data and a language model.
...
async def diagnostics_stream(
    ...
    """
    Streams introspective diagnostics for a Deep Tree Echo agent as server-sent events.
...
def format_chat_prompt(messages: List[ChatMessage]) -> str:
    ...
    Each message is prefixed according to its role ("User:", "NanEcho:", or "# System Instruction:") and the prompt ends with "NanEcho:" to signal the model to generate a response.
    """
    prompt = ""
...
def format_diagnostic_prompt(introspection_data: Dict[str, Any], focus_areas: List[str]) -> str:
    ...
    # Create the prompt
    prompt = f"""# System Instruction: You are NanEcho, an AI assistant specialized in Deep Tree Echo architecture and OpenCog systems. You're analyzing a live Deep Tree Echo agent's AtomSpace. Provide detailed introspective diagnostics based on the data below, focusing on: {', '.join(focus_areas)}.
...
def main():
    """
    Starts the NanEcho FastAPI server with command-line configuration.
    ...
    """
    parser = argparse.ArgumentParser(description="NanEcho Server")
