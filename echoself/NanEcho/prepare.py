...
This script prepares a comprehensive training corpus for NanEcho by:
1. Downloading the Deep Tree Echo architecture paper from GitHub
...
both Deep Tree Echo theory and OpenCog implementation details.
"""
...
# --- Configuration ---
COGPRIME_PAPER_URL = "https://raw.githubusercontent.com/drzo/cogprime/main/Deep Tree Echo%20-%20An%20Integrative%20Architecture%20for%20Embodied%20Artificial%20General%20Intelligence.md"
DOCUMENT_SEPARATOR = "\n\n<|endofdocument|>\n\n"
SECTION_SEPARATOR = "\n\n---\n\n"
FILE_TYPE_MARKERS = {
...
def find_repository_path(repo_name, possible_locations=None):
    ...
    if possible_locations is None:
        ...
        possible_locations = [
            # Same level as NanEcho directory
            os.path.abspath(os.path.join(script_dir, "..", "..", repo_name)),
            # One level up from NanEcho
            os.path.abspath(os.path.join(script_dir, "..", repo_name)),
            # Two levels up from NanEcho
            os.path.abspath(os.path.join(script_dir, "..", "..", "..", repo_name)),
...
def main():
    """
    Prepares the NanEcho training corpus by aggregating, annotating, tokenizing, and saving text and code files from Deep Tree Echo and OpenCog sources.
    
    This function automates the end-to-end data preparation pipeline: it downloads the Deep Tree Echo architecture paper, locates and collects relevant documentation and source files from local repositories, annotates and concatenates their contents, tokenizes the combined text using the GPT-2 tokenizer, computes and prints corpus statistics, splits the data into training and validation sets, saves them as binary files, writes metadata, and cleans up temporary files. The resulting dataset is ready for use in NanEcho model training.
    """
    ...
    if opencog_central_path:
        ...
        opencog_docs_files = [
            "README.md",
            "docs/Deep Tree Echo_Integrative_Architecture_AGI.md",
    ...
    # 4. Additional Deep Tree Echo resources if available
    if cogprime_path:
        print("\n📘 Processing Additional Deep Tree Echo Resources")
        # Main Deep Tree Echo docs
        cogprime_docs = collect_files(cogprime_path, "*.md", "Deep Tree Echo markdown")
    ...
    print("\nYou can now train NanEcho using:")
